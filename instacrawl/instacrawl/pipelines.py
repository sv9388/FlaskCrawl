...
stmt_fs = """insert into iprofile_data (iprofile_id, date, followers_count, following_count, posts_count, media_likes, engagement_rate) values ( '%s', '%s', %d, %d, %d, %d, %f )"""
    ...
    def process_item(self, item, spider):
        ...
        try:
            ...
            stmt = stmt_fs % (item['instagram_id'], date, item['followers_count'], item['following_count'], \
                            item['posts_count'], item['media_likes'], item['engagement_rate'])
            print(stmt)
            c.execute(stmt)
            self.conn.commit()
        except:
